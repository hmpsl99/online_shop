...
@router.get('/',response_model=List[product_schema.ShowProduct])
def get_products(db: Session = Depends(get_db), current_user: user_schema.SignUp = Depends(oauth2.get_current_user)):
    return product.show_all(db)
